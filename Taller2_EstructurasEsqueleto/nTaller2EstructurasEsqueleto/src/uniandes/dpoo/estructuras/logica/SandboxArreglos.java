...
public class SandboxArreglos
{
	...
	{
		int[] nuevoArregloE = new int[arregloEnteros.length];
		for (int i = 0; i < arregloEnteros.length; i++) {
			nuevoArregloE[i] = arregloEnteros[i];
		}
		return nuevoArregloE;
	}
	...
	{
		String[] nuevoArregloC = new String[arregloCadenas.length];
		for (int i = 0; i < arregloCadenas.length; i++) {
			nuevoArregloC[i] = arregloCadenas[i];
		}
		return nuevoArregloC;
	}
    ...
    {
    		for(int i = 0; i < arregloEnteros.length; i++) {
    			if (arregloEnteros[i]<0) {arregloEnteros[i] = (-1)*arregloEnteros[i];}
    		}
    }
    ...
    public int contarApariciones( String cadena )
    {
		...
		for(String palabra: arregloCadenas) {
			if (palabra.toLowerCase().equals(cadena.toLowerCase())) {contador++;}
		}
		return contador;
    }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (int i = 0; i < arregloEnteros.length; i++) {
        		if (numeros.containsKey(arregloEnteros[i])) {
        			numeros.put(arregloEnteros[i], 1);
        		}
        		else {
        			numeros.put(arregloEnteros[i], 0);
        		}}
        	for (int j: numeros.values()) {
        		contador += j;}
    		return contador;
    }
    ...
    {
    		if (arregloEnteros.length == 0 && otroArreglo.length == 0) {return true;}
		...
		if (arregloEnteros.length != otroArreglo.length) {return false;}
    		for (int i = 0; i < arregloEnteros.length; i++) {
    			...
    			for (int j = 0; j < otroArreglo.length; j++) {
    				if(arregloEnteros[i]==otroArreglo[j]) {contiene = true;}
    			}
