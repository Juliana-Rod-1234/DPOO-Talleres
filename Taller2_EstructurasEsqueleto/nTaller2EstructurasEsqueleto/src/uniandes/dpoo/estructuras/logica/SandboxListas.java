...
public class SandboxListas
{
    ...
    {
    		listaEnteros.removeIf(listaEnteros -> listaEnteros == valor);
    }
    ...
    {
    		listaCadenas.removeIf(listaCadenas -> listaCadenas.equals(cadena));
    }
    ...
    public void insertarEntero( int entero, int posicion )
    {
    		...
    		if(posicion >= listaEnteros.size() && posicion > 0) {posicion = listaEnteros.size();}
    ...
    {
    		if(0<= posicion && posicion < listaEnteros.size()) {listaEnteros.remove(posicion);}
    }
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
		...
		for (int i = 0; i < valores.length; i++) {
			listaEnteros.add((int) Math.floor(valores[i]));
		}
    }
    ...
    public int contarApariciones( int valor )
    {
    		int cont = 0;
        for(int i = 0; i < listaEnteros.size(); i++) {
        		if (listaEnteros.get(i)==valor) {cont++;}
        }
    		return cont;
    }
    ...
    public int contarApariciones( String cadena )
    {
    		int cont = 0;
        for(int i = 0; i < listaCadenas.size(); i++) {
        		if (listaCadenas.get(i).toLowerCase().equals(cadena.toLowerCase())) {cont++;}
        }
    		return cont;
    }
    ...
    public int contarEnterosRepetidos( )
    {
	    ...
	    for (int i = 0; i < listaEnteros.size(); i++) {
	    		if (numeros.containsKey(listaEnteros.get(i))) {
	    			numeros.put(listaEnteros.get(i), 1);
	    		}
	    		else {
	    			numeros.put(listaEnteros.get(i), 0);
	    		}}
    ...
    {
    		if (listaEnteros.size() == 0 && otroArreglo.length == 0) {return true;}
